...
This page is the operator contract for `emerald-exchange`. Package-specific endpoint,
...
public hostname, a routable IP, or a credential for them, and keep genuinely
environment-specific values (endpoints outside this homelab, tenant ids,
credentials, certificate paths) out of checked-in configuration entirely —
those come from `AgentConfig` / the configured secret provider at runtime.
